...
// fooBarBaz();
/*
Сперва создаём функцию-обёртку promisify, которая принимает аргументом колбэк функцию (напр. foo).
- Внутри promisify создаём новый промис, в который передаём функцию-исполнитель, принимающую параметр resolve. В этой функции-исполнителе вызывается переданная колбэк функция (напр.foo) с аргументом resolve. Т.е. вместо того, чтобы вызвать колбэк, передаваемый внутрь foo (console.log как это было ранее), мы вызываем resolve. 
*Когда внутри foo вызывается колбэк с результатом (напр. "A"), на самом деле вызывается resolve("A"), что и разрешает созданный внутри promisify промис с данными "A". Т.о. promisify оборачивает колбэк функции в промисы.
- далее необходимо использовать метод Promise.all, внутрь которого передаём массив колбэков, обёрнутых promisify, т.е. промисов.Promise.all ждёт выполнения всех этих промисов и разрешается массивом их результатов в том же порядке, в котором эти промисы были переданы. Результаты этого массива внутри Promise.all далее с помощью метода then через forEach, *поскольку это массив, передаются на вывод в консоль в том же порядке, в котором были переданы исходные колбэк функции(промисы).
*/
